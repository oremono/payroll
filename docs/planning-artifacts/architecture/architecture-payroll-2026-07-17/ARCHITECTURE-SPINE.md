...
### AD-23 — Tests are written first

- **Binds:** all — with teeth on `src/domain/**` and `src/application/**`
- **Prevents:** the SPEC's "core logic unit-tested fast and deterministically" being met *after the fact* by tests written to pass the code they describe — tests that ratify a bug instead of catching it, leaving the fairness math's determinism asserted by no one.
- **Rule:** **TDD is the development standard.** Every domain and application unit is built test-first: a failing test (**red**) precedes the code that satisfies it (**green**), then refactor. Red-before-green is XP discipline enforced in **review** — the commit sequence and PR narrative must show the test landing before or with its implementation; CI cannot prove test-first and this rule does not pretend it can. What CI *does* enforce mechanically: a **coverage floor** on `src/domain/**` and `src/application/**`, and **mutation testing** over `src/domain/**` — a surviving mutant is a test that would not have failed, i.e. exactly the gap TDD exists to close, and it fails the gate alongside lint, typecheck, and the unit suite (AD-1). The pure functional core is what makes this cheap: no DB, no clock, no network (AD-11), so the tests are fast and deterministic, as the SPEC demands. Integration tests (AD-24) are the one place DB access is allowed and live outside this domain suite, which stays clock-free and DB-free.

### AD-24 — Delivery is vertical slices, backend story before frontend story

- **Binds:** CAP-1..CAP-11
- **Prevents:** a horizontal build (every schema, then every use-case, then every screen) that demonstrates nothing until the end and lets the AD-20 answer payload be designed **twice** — once by the backend and once, differently, by the React component that consumes it — the drift AD-20 exists to kill, reintroduced through build order.
- **Rule:** the system is delivered **capability by capability**, each as a vertical slice, in small incremental commits (the assessment reads commit history). Within a capability the **backend story lands before the frontend story**: the domain + application logic, its tests (AD-23), and its **finalized AD-20 boundary payload** ship green first; the frontend story then consumes that fixed payload and adds nothing to the contract. **"Backend done" is a precise gate, not a feeling** — (1) domain + application suites green under AD-23, (2) at least one **adapter integration test** exercising the real repository against a **disposable Postgres 18** (a Neon branch or local instance, **never a mock**), and (3) the AD-20 payload finalized. Integration tests run under Vitest and sit outside the AD-23 domain suite. The **first slice is the foundational app shell** (the Structural Seed) — the import-boundary lint (AD-1), the token build step (AD-15), and the two route handlers (AD-21) — so every capability slice afterward lands on standing rails.
...
| Development & delivery | **TDD is the standard** — red → green → refactor, test-first, enforced by review plus a domain/application coverage floor and mutation testing in CI (AD-23). Delivery is **vertical slices, one capability at a time, backend story before frontend story** (AD-24), in small incremental commits — the Incubyte assessment reads commit history and test quality. |
