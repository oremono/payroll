# Engineering Manual — Salary Management for ACME HR

> The operational reference: setup, database, CI, deployment. For what the product does and why it
> is built this way, start at the [README](../README.md).
...
| `npm run seed` | Populate the database with 10,000 employees (see § Seeding) |
...
# 4. Populate the database with 10,000 employees (~4 seconds; see § Seeding)
npm run seed
...
An explicit command, never a deploy or build side effect. It writes through the same funnel as every
other write path, so it passes the same validation — currency resolved from country, no
future-dating, amount above zero.
...
- **Deliberately thin cells** of 1–3 people, so the below-threshold refusal path is reachable.
- **Planted outliers** above and below their peer median.
- **Within-group gender gaps**, seeded only into cells holding at least five of each gender — below
  that the view refuses and the gap would be invisible.
- **Gender clustering across levels**, seeded into *different* cells from the gaps. Clustering skews
  gender by level, and that skew is exactly what starves the five-per-gender threshold, so seeding
  both effects into one cell would cancel them out.

Salaries are drawn log-normally, because real salary distributions are right-skewed: a floor below
and a long tail above. Country differentials are cost-of-labour multipliers on a role/level base, and
levels progress about 15–20% apart so no level systematically out-earns the one above it.

It is byte-reproducible from a fixed seed, a fixed UUIDv7 epoch, and a fixed as-of date — all
committed constants. No wall clock, no `Math.random`. Two runs produce identical ids, amounts, and
dates. The five structural properties above are asserted by tests over the generated batch rather
than left to the draw. Parameters and reasoning: `docs/planning-artifacts/briefs/
brief-payroll-2026-07-16/addendum.md`.
...
**Production:** <https://acmesalary.vercel.app>
